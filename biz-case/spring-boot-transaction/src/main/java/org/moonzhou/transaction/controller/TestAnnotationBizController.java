...
import org.moonzhou.transaction.biz.effect.UserTransactionAnnotationBiz;
...
@AllArgsConstructor
@RestController
@RequestMapping("/api/v1/test/biz/annotation")
public class TestAnnotationBizController {

    private final UserTransactionAnnotationBiz userTransactionAnnotationBiz;
    ...
    public Result<Long> saveAll(@Validated @RequestBody UserInfoParam userInfoParam) {
        return Result.success(userTransactionAnnotationBiz.saveAll(userInfoParam));
    }
    ...
    public Result<Long> saveAllFailByFirstFail(@Validated @RequestBody UserInfoParam userInfoParam) {
        return Result.success(userTransactionAnnotationBiz.saveAllFailByFirstFail(userInfoParam));
    }
    ...
    public Result<Long> saveAllFailBySecondFail(@Validated @RequestBody UserInfoParam userInfoParam) {
        return Result.success(userTransactionAnnotationBiz.saveAllFailBySecondFail(userInfoParam));
    }
}
